...
def main():
    import filecmp
    class intraline_file_comparison:
        ...
        def main(self):
            ...
            if compare:
                ...
                while len(difference)-x>=1:
                    ...
                    x=x+1
            changes.close()
            print(f"Intraline comparison file succesfully created: {self.outputFile}")
    
    class interline_file_comparison:
        def __init__(self,file1,file2,outputFile):
            self.file1=file1
            self.file2=file2
            self.outputFile=outputFile
        ...
        def comparison(self):
            ...
            first_file_lines = Path(self.file1).read_text().splitlines()
            second_file_lines = Path(self.file2).read_text().splitlines()
    
            html_diff = difflib.HtmlDiff().make_file(first_file_lines, second_file_lines)
            Path(self.outputFile).write_text(html_diff)
        def main(self):
            compare=self.initiate_comparison()
            if compare:
                self.comparison()
                print(f"Interline comparison file succesfully created: {self.outputFile}")
    
    
    if len(sys.argv)!=5:
        print("Usage: <file1> <file2> <outputFile> <intralineComparison/interlineComparison>")
        sys.exit(0)
    file1=sys.argv[1]
    file2=sys.argv[2]
    outputFile=sys.argv[3]
    typeOfComparison=sys.argv[4]
    if typeOfComparison=="intralineComparison":
        comparison=intraline_file_comparison(r""+(file1),r""+(file2),r""+outputFile)
        comparison.main()
    elif typeOfComparison=="interlineComparison":
        comparison=interline_file_comparison(r""+(file1),r""+(file2),r""+outputFile)
        comparison.main()
main()
